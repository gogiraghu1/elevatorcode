...
import com.rgogi.ecs.interfaces.ElevatorControlSystemFactory;
...
/**
 * This class is responsible for initializing the configuration for number of elevators and number of floors 
 * also to maintain the central queue for pickup requests 
*  To delegate the pickup request to each elevator after checking the state of the available elevator
 * 
 * 
 * @author rgogi
 *
    ...
    for (Elevator currElevator : elevators){
      ...
      switch (currElevator.status()){
        ...
        case ELEVATOR_OCCUPIED:
          ...
          if (currElevator.state() == ElevatorState.ELEVATOR_UP)
          break;
      }
    }
  }
